...
def lqr_observer(A,C,Q,R):
    ...
    #print('eigVals: ', eigVals)
    return K.T
# a function that implements the cost function
def lqr_cost_function(x,u):
    #print('x: ', x)
    #print('u: ', u)
    return x.T*Q*x + u.T*R*u
# pass the matrix z as the input matrix x of the lqr controller and cost function
x = np.matrix([[z],[0]])
#print('x: ', x)
#print('x.shape: ', x.shape)
#print('u.shape: ', u.shape)
#print('lqr_cost_function(x,u): ', lqr_cost_function(x,u))
# define the matrices A, B, C, Q and R for the lqr controller and observer
A = np.matrix([[0, 1], [0, 0]])
B = np.matrix([[0], [1]])
C = np.matrix([[1, 0]])
Q = np.matrix([[1, 0], [0, 1]])
R = np.matrix([[1]])
# compute the lqr controller and observer gains (K and L)
K = lqr_controller(A,B,Q,R)
L = lqr_observer(A,C,Q,R)
# minimize the cost function and generate the required output for stable control
u = -K*x
y = C*x
#print('u: ', u)
#print('y: ', y)
# pass the output obtained as scaled volatge to a bldc motor connected to an arduino board
board.analog[0].mode = pyfirmata.PWM
board.analog[0].write(y[0,0]/5)
